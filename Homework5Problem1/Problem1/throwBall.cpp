...
/*
	Maeda Hanafi
	CSC431 Homework 5 Problem 1
	Modified for a box to move along with the ball
*/
#include <cmath>
#include <cstdlib>
#include <iostream>
...
static float sphereInitX=-10.0, sphereInitY=0.0, sphereInitZ=-40.0;//Initial coordinates of the sphere
static float sphereX=-10.0, sphereY=0.0+cubeSize/2, sphereZ=0.0;//Shifts of the sphere 
static float sphereCurrY = sphereInitY,cubeCurrY=sphereY-cubeSize;//Y Coordinates of the cube and sphere
...
void drawScene(void)
{
   ...
   // Place cube in frustum and correct location.
   glTranslatef(sphereInitX,sphereInitY,sphereInitZ);
  
   // Apply equations of motion to transform box.
   glTranslatef(sphereX, 0.0 , sphereZ);  
   cubeCurrY=sphereInitY;
...
{
   if(sphereCurrY<cubeCurrY+cubeSize/2 && t>3){
			isAnimate = 0;
   }
   if (isAnimate) 
   {
